...
exports.load = function(lessDependencies, optimizerContext, pluginConfig, callback) {
        ...
        parts.forEach(function(part) {
            if (part.isImport()) {
                ...
                if (urlRegExp.test(url)) {
                    // Path is already an absolute URL (e.g. "http://...")
                    return;
                }

                urlTasks.push(function(callback) {
                    urlResolver(url, dirname, optimizerContext, function(err, path) {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        optimizer.optimizeResource(path, {optimizerContext: optimizerContext}, function(err, optimizedResource) {
                            if (err) {
                                return callback(err);
                            }
