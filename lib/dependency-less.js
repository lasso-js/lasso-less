...
module.exports = function create(config, optimizer) {
    ...
    return {
        ...
        read: function(optimizerContext, callback) {
            ...
            var lessImportDependencies = optimizerContext.data.lessImportDependencies;
            ...
            loader.load(lessDependencies, optimizerContext, config, function(err, lessCode) {
                if (err) {
                    return callback(err);
                }
        ...
        // to tell the optimizer-resolve-css-urls to not bother transforming the
